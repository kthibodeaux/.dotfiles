...
map('n', '<leader>fm', ':silent execute "!tmux split-window ranger " . expand(\'%:p:h\')<CR>', { noremap = true, desc = 'open ranger' })
map('n', '<leader>gh', '<Plug>GitGoBack', { noremap = true, desc = 'history' })
map('n', '<leader>m', '<Plug>OpenDbMigrate', { noremap = true, desc = 'migrations' })
